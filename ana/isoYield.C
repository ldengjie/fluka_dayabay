{
    ...
    for( int j=0 ; j<1 ; j++ )
    {
        ...
        //for( int i=1 ; i<167; i++ )
        {
            ...
            nameStr=Form("/afs/ihep.ac.cn/users/l/lidj/largedata/flukaWork/JUNO/data/%s/rootFile/fluSim_%06d.root",dataVer[j].c_str(),i);
            ...
            }else
            {
                ...
                {
                    mt->GetEntry(u);
                    if(muOwsTrackLength==0) continue;
                ...
                for( int r=0 ; r<neutnum ; r++ )
                {
                    ...
                    if(neuCapVolumeName== 5  &&neuInitVolumeName!= 5 ) neuNumIn++;
                    if(neuCapVolumeName!= 5  && neuInitVolumeName== 5 ) neuNumOut++;

                    //if( neuCapVolumeName== 5  )
                    //{
                    //neuNum++;
                    //if(neuInitVolumeName!= 5 ) neuNumIn++;
                    //}
                    //if(neuCapVolumeName!= 5  && neuInitVolumeName== 5 ) neuNumOut++;
                ...
                for( int is=0 ; is<isotnum ; is++ )
                {
                    ...
                    if( isoDecayVolume== 5  )
                    {
                        ...
                        //if(isoOriginVolumeNumber== 5 &&isoInitVolume!= 5 ) isoNumIn[isoZ][isoA]++;//==zero
                        if(isoInitVolume!= 5 ) isoNumIn[isoZ][isoA]++;
                    }
                    //if(isoOriginVolumeNumber== 5  && isoDecayVolume!= 5 &&isoInitVolume== 5 ) isoNumOut[isoZ][isoA]++;
                    if(isoDecayVolume!= 5 &&isoInitVolume== 5 ) isoNumOut[isoZ][isoA]++;
                }
